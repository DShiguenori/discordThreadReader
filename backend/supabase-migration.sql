...
-- Create the prompts table for storing AI prompt configuration
...
DROP POLICY IF EXISTS "Allow public access" ON prompts;

-- Create a policy that allows all operations for anonymous and authenticated users
CREATE POLICY "Allow public access" ON prompts
  FOR ALL
  USING (true)
  WITH CHECK (true);

-- Add comments for documentation
COMMENT ON TABLE prompts IS 'Stores AI prompt configuration for thread summarization';
COMMENT ON COLUMN prompts.key IS 'Unique identifier for the prompt (default: "default")';
COMMENT ON COLUMN prompts.prompt IS 'The prompt template used for OpenAI API calls';
